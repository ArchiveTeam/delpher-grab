...
allowed = function(url, parenturl)
  if string.match(url, "'+")
      or string.match(url, "[<>\\%*%$;%^%[%],%(%){}]")
      or string.match(url, "%%5Bimageviewer%%5D%%5B[srh]%%5D=") then
    return false
  end
  ...
  if string.match(string.gsub(url, "%%3[aA]", ":"), item_value) then
    return true
  end
...
wget.callbacks.get_urls = function(file, url, is_css, iri)
  ...
  if allowed(url, nil) and status_code == 200
    and not string.match(url, "^https?://imageviewer%.kb.nl/")
    and not string.match(url, "^https?://resolver%.kb%.nl/.+:image$")
    and not string.match(url, "^https?://resolver%.kb%.nl/.+:pdf$") then
    html = read_file(file)
    match = string.match(string.gsub(url, "%%3[aA]", ":"), "(" .. item_value .. "[a-zA-Z0-9:]+)")
    if match then
      check("https://resolver.kb.nl/resolve?urn=" .. match)
    end
