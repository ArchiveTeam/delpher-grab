...
LUA_SHA1 = get_hash(os.path.join(CWD, 'delpher.lua'))
...
class WgetArgs(object):
    def realize(self, item):
        wget_args = [
            ...
            '--timeout', '30',
            '--tries', 'inf',
            '--domains', 'delpher.nl,kb.nl',
            '--span-hosts',
            '--waitretry', '30',
            '--warc-file', ItemInterpolation('%(item_dir)s/%(warc_file_base)s'),
            '--warc-header', 'operator: Archive Team',
            '--warc-header', 'delpher-dld-script-version: ' + VERSION,
            '--warc-header', ItemInterpolation('delpher-item: %(item_name)s'),
        ...
        if item_type == 'urn':
            wget_args.extend(['--warc-header', 'delpher-id: ' + str(item_value)])
            wget_args.extend(['--warc-header', 'kb-id: ' + str(item_value)])
            wget_args.append('https://resolver.kb.nl/resolve?urn={}:mpeg21'.format(item_value))
            wget_args.append('https://resolver.kb.nl/resolve?urn={}'.format(item_value))
...
project = Project(
    title = 'Delpher',
    project_html = '''
    <img class="project-logo" alt="logo" src="https://www.archiveteam.org/images/3/32/Delpher-logo.png" height="50px"/>
    <h2>delpher.nl <span class="links"><a href="https://depher.nl/">Website</a> &middot; <a href="http://tracker.archiveteam.org/delpher/">Leaderboard</a></span></h2>
    '''
)
...
pipeline = Pipeline(
    ...
    GetItemFromTracker('http://%s/%s' % (TRACKER_HOST, TRACKER_ID), downloader,
        VERSION),
    PrepareDirectories(warc_prefix='delpher'),
    WgetDownload(
        WgetArgs(),
        max_tries=2,
